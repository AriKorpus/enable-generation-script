...
// Add Air Venting Holes In Forearm
...
HandScale = ArmCircumferenceScale * (AdditionalHandScale/100);
...
module print_part( ) {
    ...
    if(part == "HingeThumbKnukle") { scale([HandScale,HandScale,HandScale]) cube([31, 2.5, 6.25]); }
    
    if(part == "LatchHinge") { scale([ArmCircumferenceScale,ArmCircumferenceScale,ArmCircumferenceScale]) cube([12, 2.5, 6.25]); }
...
module MakeWristCompressionBushing() {
    ...
    WristDiameter = 53.6 * ArmCircumferenceScale;
...
module MakeArm(PieceNumber) {
    ...
    difference(){
        ...
        if(PieceNumber == 1) {
            ...
            cylinder(d = WristBoltDia - 0.05, h = 27.00 * ArmScale + 50 * ArmScale, center=true, $fn=30);
            
            // cut small hole for wrist bolt 
            translate([2.513 * ArmCircumferenceScale, 14.753 * ArmCircumferenceScale, -(21.5* ArmScale) + (30* ArmScale)/2 ]) cylinder(d = WristBoltDia - 4, h = 40.00 * ArmScale + 60 * ArmScale, center=true, $fn=30);
        }
             ...
             if(GripLatch == "Yes")
              {
                  ...
                  translate([-19.158* ArmCircumferenceScale, 53.056 * ArmCircumferenceScale, -(187.289* ArmScale)]) rotate([-90,0,25]) LatchHole();
              }
        }
    ...
    if(PieceNumber == 1) {
        difference(){
            ...
            thread_in(WristBoltDia, 23 * ArmScale + 40 * ArmScale);
    ...
    if(PieceNumber == 2 || ArmPieces == 1) {
      ...
      if(GripLatch == "Yes")
      {
           translate([-19.158* ArmCircumferenceScale, 53.056 * ArmCircumferenceScale, -(187.289* ArmScale)]) rotate([-90,0,25]) LatchBase();
      }  
    }
...
module MakePalm() {
    ...
    difference(){
        ...
        thread_in(WristBoltDia, 23 * HandScale + 40 * ArmScale);
          ...
          // truncate threads length inside palm
        translate([2.513 * HandScale - 100, 14.753 * HandScale - 100, (-6 * HandScale)-0.5])
        cube(200);
        // truncate threads length to wrist end
        translate([2.513 * HandScale - 100, 14.753 * HandScale - 100, -200-(21.5* HandScale) + 0.1]) cube(200);
...
module Arm() {scale([ArmCircumferenceScale,ArmCircumferenceScale,ArmScale]) import("o_Arm.stl", convexity=3); }
